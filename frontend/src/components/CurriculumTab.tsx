...
export default function CurriculumTab({
  ...
  return (
    <div className="max-w-4xl animate-in fade-in duration-300">
      ...
      <p className="text-slate-400 mb-8">Design your course structure by adding modules and lessons.</p>
      
      <div className="space-y-6 mb-8">
        {modules.length === 0 ? (
          <div className="p-8 border-2 border-dashed border-white/10 rounded-2xl text-center">
            <p className="text-slate-500">Your curriculum is empty. Add a module to get started!</p>
          </div>
        ) : (
          modules.map((module, index) => (
            <div key={module.id} className="bg-slate-900/80 border border-white/10 rounded-xl overflow-hidden group">
              <div className="p-5 flex items-center justify-between bg-slate-900 border-b border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <span className="text-xs font-bold text-indigo-400 mb-1 block">Module {index + 1}</span>
              ...
              <div className="p-4 space-y-2">
                {!module.lessons || module.lessons.length === 0 ? (
                  ...
                  module.lessons.map((lesson, lIndex) => (
                    <div key={lesson.id} className="flex items-center gap-4 bg-slate-950 p-4 rounded-lg border border-white/5 hover:border-white/10 transition-colors">
                      ...
                      <div className="flex gap-3 items-center border-l border-white/5 pl-4">
                        <div className="flex gap-2">
                          {lesson.video_url && <Video className="h-4 w-4 text-slate-500" title="Has Video" />}
                          {lesson.content && <FileText className="h-4 w-4 text-slate-500" title="Has Text Content" />}
                        </div>
                        <button
      ...
      <form onSubmit={handleCreateModule} className="bg-slate-900/50 border border-white/10 p-6 rounded-2xl">
